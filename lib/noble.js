const NobleEvents = require('./noble-events'),
      util = require('util'),
      PeripheralDb = require('./peripheraldb'),
      debug = require('debug')('noble')

const Peripheral = require('./peripheral');
class Noble extends NobleEvents {
  constructor(bindings, peripheralDb = null) {
    super()
    ...
    this.peripherals = peripheralDb ? peripheralDb : new PeripheralDb();
  ...
  startScanning(serviceUuids, allowDuplicates, active = true) {
    const scan = (state) => {
      if (state !== 'poweredOn') {
        ...
        this._bindings.startScanning(serviceUuids, allowDuplicates, active);
      }
    };
  ...
  onDiscover(uuid, address, addressType, connectable, advertisement, rssi) {
    ...
    let previouslyDiscoverd = false
    if (!peripheral) {
      peripheral = new Peripheral(uuid, address, addressType, connectable, advertisement, rssi);

      this.peripherals.add(peripheral)
      previouslyDiscoverd = true
    } else {
  ...
  connect(peripheral, parameters) {
    // we may not have this peripheral if we are being cross-connected from another device
    this.peripherals.add(peripheral)

    // send to the binding
    this._bindings.connect(peripheral.uuid, peripheral.addressType, parameters);
  }
  ...
  onConnect(peripheralUuid, error) {
    ...
    this.emit('connect', peripheralUuid, peripheral, error)
  }
  ...
  onDisconnect(peripheralUuid) {
    ...
    this.emit('disconnect', peripheralUuid, peripheral)
  }
  ...
  encrypt(peripheralUuid){
    this._bindings.encrypt(peripheralUuid);
  }
  ...
  discoverCharacteristics(peripheralUuid, serviceUuid) {
    this._bindings.discoverCharacteristics(peripheralUuid, serviceUuid);
  }
  ...
  notify(peripheralUuid, serviceUuid, characteristicUuid, notify, options = {}) {
    this._bindings.notify(peripheralUuid, serviceUuid, characteristicUuid, notify, options);
  }
