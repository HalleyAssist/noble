...
class Service extends events.EventEmitter {
  constructor(perhipheral, uuid, startHandle = null, endHandle = null){
    super()
    this.peripheral = perhipheral
  ...
  static fromDump(entry, peripheral){
    const ret = new Service(peripheral, entry.uuid, entry.startHandle, entry.endHandle)
    if(entry.characteristics) ret.characteristics = entry.characteristics.map(s=>Characteristic.fromDump(s, peripheral, ret))
    return ret
  }
}
...
Service.prototype.discoverIncludedServicesAsync = eventRequest.promisify(function(noble, serviceUuids){
  return noble.discoverIncludedServices(
    this.peripheral.id,
    this.uuid,
    serviceUuids
  );
...
Service.prototype.discoverCharacteristicsAsync = eventRequest.promisify(function(noble){
  return noble.discoverCharacteristics(
    this.peripheral.id,
    this.uuid
  );
