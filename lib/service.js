...
class Service extends events.EventEmitter {
  ...
  onCharacteristicsDiscover(characteristics) {
    const ret = []
    for(const cIn of characteristics){
      let cObj = this.getCharacteristic(cIn.uuid)
      if(!cObj) cObj = this.addCharacteristic(cIn)
  ...
  onCharacteristicsDiscovered(characteristics) {
    this.emit('characteristicsDiscovered', characteristics);
  }
  ...
  onNotify(characteristicUuid, state) {
    const c = this._getCharacteristic(characteristicUuid)
    return c.onNotify(state)
  }
