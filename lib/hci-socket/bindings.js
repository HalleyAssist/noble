...
NobleBindings.prototype.onLeConnComplete = function (status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if ((role !== undefined && role !== 0) || !this._pendingConnection) {
    this._logger(((role !== undefined && role !== 0) ? 'not master' : 'not connecting') + ' ignore connection result')
    if(status == 0) this._hci.disconnect(handle)
    return;
  }

  try {
    let uuid = null;
    if(!address) uuid = this._pendingConnection.peripheralUuid;
    else uuid = address.split(':').join('').toLowerCase();

    if(addressType === undefined){
      addressType = this._pendingConnection.addressType;
    }
    console.log('onLeConnComplete', {uuid, addressType, status})
...
NobleBindings.prototype.onRssiRead = function (handle, rssi) {
  this.emit('rssiUpdate', this._handles.getUuid(handle), rssi);
};
