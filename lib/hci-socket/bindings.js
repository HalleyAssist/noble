...
NobleBindings.prototype.onLeConnComplete = function (status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (status === 0) {
    ...
    console.log(`connected ${uuid} as ${handle}`)
    if(this._handles[uuid]){
      this._logger(`noble warning: disconnecting old instance at ${existingHandle} as we have a new connection at ${handle}`);
      const existingHandle = this._handles[uuid]
    ...
    const gatt = new Gatt(address, aclStream, this._noble);
  ...
  console.log('connect', {uuid, error})
...
NobleBindings.prototype.onDisconnComplete = function (handle, reason) {
  ...
  console.log(`disconnected ${uuid} as ${handle}`)
  ...
  console.log('disconnect', {uuid, reason})
...
NobleBindings.prototype.addService = function (peripheralUuid, service) {
  ...
  this._logger(`noble warning: unknown peripheral ${peripheralUuid} during addService`);
  return false
};
