...
const Hci = function (options) {
  ...
  this._deviceId = options.deviceId != null
    ...
    : process.env.NOBLE_HCI_DEVICE_ID
      ...
      : undefined;
  this._logger = Debug(`noble:hci:hci${this._deviceId === undefined ? '': this._deviceId}`);
...
Hci.prototype.init = function (options) {
  ...
  if (this._userChannel) {
    ...
    this._timer = setInterval(this.pollIsDevUp.bind(this), 1000);
  }
};

Hci.prototype._initDev = function(){
...
Hci.prototype.setSocketFilter = function () {
  ...
  this._logger(`setting filter to: ${filter.toString('hex')}`);
...
Hci.prototype.setEventMask = function () {
  ...
  this._logger(`set event mask - writing: ${cmd.toString('hex')}`);
...
Hci.prototype.reset = function () {
  ...
  this._logger(`reset - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readLocalVersion = function () {
  ...
  this._logger(`read local version - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readBufferSize = function () {
  ...
  this._logger(`read buffer size - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readBdAddr = function () {
  ...
  this._logger(`read bd addr - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.setLeEventMask = function () {
  ...
  this._logger(`set le event mask - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readLeBufferSize = function () {
  ...
  this._logger(`le read buffer size - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readLeHostSupported = function () {
  ...
  this._logger(`read LE host supported - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.writeLeHostSupported = function () {
  ...
  this._logger(`write LE host supported - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};

Hci.prototype.setScanParameters = function (interval = 0x0020, window = 0x0018, active = true) {
  ...
  // data
  cmd.writeUInt8(active ? 0x01 : 0x00, 4); // type: 0 -> passive, 1 -> active
  cmd.writeUInt16LE(interval, 5); // interval, ms * 1.6
  cmd.writeUInt16LE(window, 7); // window, ms * 1.6
  cmd.writeUInt8(0x00, 9); // own address type: 0 -> public, 1 -> random
  cmd.writeUInt8(0x00, 10); // filter: 0 -> all event types

  console.log(`set scan parameters - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.setScanEnabled = function (enabled, filterDuplicates) {
  ...
  console.log(`set scan ${enabled?'enabled':'disabled'} - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};

Hci.prototype.createLeConn = function (peripheralUuid, addressType, parameters = {}) {
  let { minInterval = 0x0006, maxInterval = 0x000c, latency = 0x0000, timeout = 0x00c8, interval, window } = parameters;
  if(!interval || interval < minInterval){
    interval = 0x0020
  }
  if(!window || window > interval){
    window = Math.ceil(interval / 2);
  }
  ...
  // data
  cmd.writeUInt16LE(interval, 4); // interval
  cmd.writeUInt16LE(window, 6); // window
  cmd.writeUInt8(0x00, 8); // initiator filter

  cmd.writeUInt8(addressType === 'random' ? 0x01 : 0x00, 9); // peer address type

  bufferReverse(Buffer.from(peripheralUuid, "hex")).copy(cmd, 10) // peer address

  console.log({peripheralUuid, addressType, reversed: bufferReverse(Buffer.from(peripheralUuid, "hex")).toString('hex')})
  ...
  this._logger(`create le conn - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.connUpdateLe = function (handle, minInterval, maxInterval, latency, supervisionTimeout) {
  ...
  this._logger(`conn update le - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.cancelConnect = function () {
  ...
  this._logger('cancel le conn - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.startLeEncryption = function (handle, random, diversifier, key) {
  ...
  this._logger(`start le encryption - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.disconnect = function (handle, reason) {
  ...
  this._logger(`disconnect - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.readRssi = function (handle) {
  ...
  this._logger(`read rssi - writing: ${cmd.toString('hex')}`);
  this._socket.write(cmd);
};
...
Hci.prototype.writeAclDataPkt = function (handle, cid, data) {
  ...
  this._logger(`push to acl queue: ${first.toString('hex')}`);
  ...
  while (data.length > 0) {
    ...
    this._logger(`push fragment to acl queue: ${frag.toString('hex')}`);
...
Hci.prototype.flushAcl = function () {
  ...
  this._logger(`flush - pending: ${pendingPackets()} queue length: ${this._aclQueue.length}`);
  ...
  while (this._aclQueue.length > 0 && pendingPackets() < this._aclBuffers.num) {
    ...
    this._logger(`write acl data packet - writing: ${packet.toString('hex')}`);
    this._socket.write(packet);
  }
};

Hci.prototype.onSocketData = function (data) {
  this._logger(`onSocketData: ${data.toString('hex')}`);
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (subEventType === EVT_DISCONN_COMPLETE) {
      ...
      this._logger(`\t\tcmd = ${cmd}`);
      this._logger(`\t\tstatus = ${status}`);
      this._logger(`\t\tresult = ${result.toString('hex')}`);
      ...
      this._logger(`\t\tLE meta event type = ${leMetaEventType}`);
      this._logger(`\t\tLE meta event status = ${leMetaEventStatus}`);
      this._logger(`\t\tLE meta event data = ${leMetaEventData.toString('hex')}`);
    ...
    if (ACL_START === flags) {
      ...
      this._logger(`\t\tcid = ${cid}`);

      if (length === pktData.length) {
        this._logger(`\t\thandle = ${handle}`);
        this._logger(`\t\tdata = ${pktData.toString('hex')}`);
    ...
    if (cmd === LE_SET_SCAN_ENABLE_CMD) {
      ...
      this._logger('\t\t\tLE enable scan command');
      this._logger(`\t\t\tenable scanning = ${enable}`);
      this._logger(`\t\t\tfilter duplicates = ${filterDuplicates}`);
...
Hci.prototype.onSocketError = function (error) {
  this._logger(`onSocketError: ${error.message}`);
...
Hci.prototype.processCmdCompleteEvent = function (cmd, status, result) {
  if (cmd === RESET_CMD) {
    ...
    this._logger(`address = ${this.address}`);
    ...
    if (status === 0) {
      ...
      if (aclLength === 0 || aclNum === 0) {
        this._logger('using br/edr buffer size');
        this.readBufferSize();
      } else {
        this._logger(`le buffer size: length = ${aclLength}, num = ${aclNum}`);
    ...
    this._logger(`buffer size: length = ${aclLength}, num = ${aclNum}`);
    this._aclBuffers.length = aclLength;
    this._aclBuffers.num = aclNum;
  }
};
...
Hci.prototype.processLeConnComplete = function (status, data) {
  ...
  this._logger(`\t\t\thandle = ${handle}`);
  this._logger(`\t\t\trole = ${role}`);
  this._logger(`\t\t\taddress type = ${addressType}`);
  this._logger(`\t\t\taddress = ${address}`);
  this._logger(`\t\t\tinterval = ${interval}`);
  this._logger(`\t\t\tlatency = ${latency}`);
  this._logger(`\t\t\tsupervision timeout = ${supervisionTimeout}`);
  this._logger(`\t\t\tmaster clock accuracy = ${masterClockAccuracy}`);
...
Hci.prototype.processLeAdvertisingReport = function (count, data) {
  try {
    for (let i = 0; i < count; i++) {
      ...
      this._logger(`\t\t\ttype = ${type}`);
      this._logger(`\t\t\taddress = ${address}`);
      this._logger(`\t\t\taddress type = ${addressType}`);
      this._logger(`\t\t\teir = ${eir.toString('hex')}`);
      this._logger(`\t\t\trssi = ${rssi}`);
