  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      ...
      for (i = 0; i < services.length; i++) {
        ...
        const s = this._noble.getService(this._id, uuid)
        Object.assign(s, services[i])
      }
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === service.endHandle) {
      ...
      for (i = 0; i < characteristics.length; i++) {
        ...
        const c = this._noble.getCharacteristic(this._id, serviceUuid, characteristic.uuid);
        Object.assign(c, characteristic)
