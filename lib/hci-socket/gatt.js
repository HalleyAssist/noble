...
Gatt.prototype.onAclStreamData = function (cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    const services = this._noble.getPeripheral(this._address).services
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), data => {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          ...
          this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error(`Invalid write response`))
        }
      });
    }else{
      this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error(`Invalid read response`))
    }
  });
};
