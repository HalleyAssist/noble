...
const Gatt = function (address, aclStream, noble) {
  this._address = address;
  this._id = address.replace(/:/g,'')
    ...
    for (const s of services) {
      ...
      for (const c of s.characteristics) {
        if (c.valueHandle === valueHandle) {
          this.emit('notification', this._address, s.uuid, c.uuid, valueData);
        }
      }
    }
...
Gatt.prototype.discoverServices = function (uuids) {
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      this.emit('servicesDiscover', this._address, services);
    } else {
...
Gatt.prototype.notify = function (serviceUuid, characteristicUuid, notify, options = {}) {
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_CLIENT_CHARAC_CFG_UUID), data => {
    if(data.length < 4) {
      this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error(`Invalid read response (got length: ${data.length})`))
      return
    }
    const opcode = data[0];
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      if(!useNotify && !useIndicate) {
        this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error('Unable to use notify or indicate as characteristic does not support'))
        return
      }
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), data => {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, {useNotify, useIndicate});
        }else{
          this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error(`Invalid write response (got opcode: ${opcode})`))
        }
      });
    }else{
      this.emit('notify', this._address, serviceUuid, characteristicUuid, notify, new Error(`Invalid read response (got opcode: ${opcode})`))
    }
  });
};
