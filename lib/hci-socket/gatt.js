  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      ...
      for (i = 0; i < services.length; i++) {
        ...
        const s = this._noble.getService(this.address, uuid)
...
Gatt.prototype.discoverIncludedServices = function (serviceUuid, uuids) {
  const service = this._noble.getService(this.address, serviceUuid)
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === service.endHandle) {
      ...
      for (i = 0; i < characteristics.length; i++) {
        ...
        const c = this._noble.getCharacteristic(this.address, serviceUuid, characteristic.uuid);
...
Gatt.prototype.read = function (serviceUuid, characteristicUuid) {
  const characteristic = this._noble.getCharacteristic(this.address, serviceUuid, characteristicUuid);
...
Gatt.prototype.readValue = function (serviceUuid, characteristicUuid, descriptorUuid) {
  const descriptor = this._noble.getDescriptor(this.address, serviceUuid, characteristicUuid, descriptorUuid)
