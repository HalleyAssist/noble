const debug = require('debug')('noble:hci:att');
...
const ATT_OP_FIND_TYPE_REQ = 0x06;
const ATT_OP_FIND_TYPE_RESP = 0x07;
...
const Gatt = function (uuid, aclStream, noble) {
  this._id = uuid
...
Gatt.prototype.onAclStreamData = function (cid, data) {
  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    debug(`${this._id}: echo ... echo ... echo ...`);
  } else if (data[0] % 2 === 0) {
    if (process.env.NOBLE_MULTI_ROLE) {
      debug(`${this._id}: multi-role flag in use, ignoring command meant for peripheral role.`);
    } else {
      const requestType = data[0];
      debug(`${this._id}: replying with REQ_NOT_SUPP to 0x${requestType.toString(16)}`);
    ...
    this.emit('handleNotify', this._id, valueHandle, valueData);

    if (data[0] === ATT_OP_HANDLE_IND) {
      this._queueCommand(this.handleConfirmation(), null, () => {
        this.emit('handleConfirmation', this._id, valueHandle);
      });
    }

    const peripheral = this._noble.getPeripheral(this._id)    
    ...
    for (const s of services) {
      ...
      for (const c of s.characteristics) {
        if (c.valueHandle === valueHandle) {
          this.emit('notification', this._id, s.uuid, c.uuid, valueData);
        }
      }
    }
  } else if (!this._currentCommand) {
    debug(`${this._id}: uh oh, no current command`);
  } else {
    if (data[0] === ATT_OP_ERROR &&
        ...
        this._security !== 'medium') {
          this._encryptCommand = this._currentCommand;
          this._currentCommand = null
      this._aclStream.encrypt();
      return;
    }

    debug(`${this._id}: read: ${data.toString('hex')}`);

    this._currentCommand.callback(data);

    this._currentCommand = null;

    this._pushCommandQueue();
  }
};

Gatt.prototype._pushCommandQueue = function(){
...
Gatt.prototype.onAclStreamEncrypt = function (encrypt) {
  if (encrypt) {
    ...
    const ec = this._encryptCommand
    if(ec){
      this._encryptCommand = null
      this._queueCommand(ec.buffer, ec.callback, ec.writeCallback, true);
    }
  }
};

Gatt.prototype.onAclStreamEncryptFail = function () {
  debug('Encryption setup failed')
};
...
Gatt.prototype.writeAtt = function (data) {
  debug(`${this._id}: write: ${data.toString('hex')}`);
...
Gatt.prototype._queueCommand = function (buffer, callback, writeCallback, top = false) {
  const record = {
    buffer: buffer,
    callback: callback,
  ...
  if(top){
    this._commandQueue.unshift(record)
  }else{
    this._commandQueue.push(record);
  }
...
Gatt.prototype.exchangeMtu = function (mtu) {
  this._queueCommand(this.mtuRequest(mtu), (data) => {
    ...
    if (opcode === ATT_OP_MTU_RESP) {
      ...
      debug(`${this._id}: new MTU is ${newMtu}`);

      this._mtu = newMtu;
    }

    this.emit('mtu', this._id, this._mtu);
  });
};
...
Gatt.prototype.discoverServices = function (uuids) {
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      this.emit('servicesDiscover', this._id, services);
    } else {
...
Gatt.prototype.discoverIncludedServices = function (serviceUuid, uuids) {
  const service = this._noble.getService(this._id, serviceUuid)
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || includedServices[includedServices.length - 1].endHandle === service.endHandle) {
      ...
      this.emit('includedServicesDiscover', this._id, service.uuid, includedServiceUuids);
    } else {
...
Gatt.prototype.discoverCharacteristics = function (serviceUuid) {
  const service = this._noble.getService(this._id, serviceUuid)
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === service.endHandle) {
      ...
      for (i = 0; i < characteristics.length; i++) {
        ...
        const characteristic = {
          ...
          startHandle: characteristics[i].startHandle,
          valueHandle: characteristics[i].valueHandle
        };

        // work around name-clash of numeric vs. string-array properties field:
        characteristic.propsDecoded = characteristic.properties;
        characteristic.rawProps = properties;

        if (i !== 0) {
          characteristicsDiscovered[i - 1].endHandle = characteristic.startHandle - 1;
        }
      ...
      this.emit('characteristicsDiscover', this._id, serviceUuid, characteristicsDiscovered);
    } else {
...
Gatt.prototype.read = function (serviceUuid, characteristicUuid) {
  const characteristic = this._noble.getCharacteristic(this._id, serviceUuid, characteristicUuid);
  ...
  const callback = (data) => {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('read', this._id, serviceUuid, characteristicUuid, readData);
      }
    } else {
      this.emit('read', this._id, serviceUuid, characteristicUuid, readData);
    }
...
Gatt.prototype.write = function (serviceUuid, characteristicUuid, data, withoutResponse) {
  ...
  if (withoutResponse) {
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, true), null, () => {
      this.emit('write', this._id, serviceUuid, characteristicUuid);
    });
  ...
  const prepareWriteCallback = (dataChunk) => {
    return (resp) => {
      ...
      if (opcode !== ATT_OP_PREPARE_WRITE_RESP) {
        debug(`${this._id}: unexpected reply opcode %d (expecting ATT_OP_PREPARE_WRITE_RESP)`, opcode);
      } else {
        ...
        if (resp.length !== expectedLength) {
          ...
          debug(`${this._id}: unexpected prepareWriteResponse length %d (expecting %d)`, resp.length, expectedLength);
        }
      }
    };
  };
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_SERVER_CHARAC_CFG_UUID), data => {
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), data => {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          this.emit('broadcast', this._id, serviceUuid, characteristicUuid, broadcast);
        }
      });
    }
  });
...
Gatt.prototype.notify = function (serviceUuid, characteristicUuid, notify, options = {}) {
  const characteristic = this._noble.getCharacteristic(this._id, serviceUuid, characteristicUuid);
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_CLIENT_CHARAC_CFG_UUID), data => {
    if(data.length < 4) {
      this.emit('notify', this._id, serviceUuid, characteristicUuid, notify, new Error(`Invalid read response (got length: ${data.length})`))
      return
    }
    const opcode = data[0];
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      const handle = data.readUInt16LE(2);
      let value = data.length < 6 ? 0 : data.readUInt16LE(4);

      const useNotify = (options.notify === undefined) ? characteristic.properties.includes('notify') : options.notify;
      const useIndicate = (options.indicate === undefined) ? characteristic.properties.includes('indicate') : options.indicate;


      if(!useNotify && !useIndicate) {
        this.emit('notify', this._id, serviceUuid, characteristicUuid, notify, new Error('Unable to use notify or indicate as characteristic does not support'))
        return
      }
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), data => {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          this.emit('notify', this._id, serviceUuid, characteristicUuid, notify, {useNotify, useIndicate});
        }else{
          this.emit('notify', this._id, serviceUuid, characteristicUuid, notify, new Error(`Invalid write response (got opcode: ${opcode})`))
        }
      });
    }else{
      this.emit('notify', this._id, serviceUuid, characteristicUuid, notify, new Error(`Invalid read response (got opcode: ${opcode})`))
    }
  });
};
  ...
  const callback = data => {
    ...
    if (opcode !== ATT_OP_FIND_INFO_RESP || descriptors[descriptors.length - 1].handle === characteristic.endHandle) {
      ...
      for (i = 0; i < descriptors.length; i++) {
        ...
        const d = characteristic.getDescriptor(descriptors[i].uuid)
        Object.assign(d, descriptors[i])
      }

      this.emit('descriptorsDiscover', this._id, serviceUuid, characteristicUuid, descriptorUuids);
    } else {
...
Gatt.prototype.readValue = function (serviceUuid, characteristicUuid, descriptorUuid) {
  const descriptor = this._noble.getDescriptor(this._id, serviceUuid, characteristicUuid, descriptorUuid)
  ...
  const callback = (data) => {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('valueRead', this._id, serviceUuid, characteristicUuid, descriptorUuid, readData);
      }
    } else {
      this.emit('valueRead', this._id, serviceUuid, characteristicUuid, descriptorUuid, readData);
    }
...
Gatt.prototype.writeValue = function (serviceUuid, characteristicUuid, descriptorUuid, data) {
  ...
  this._queueCommand(this.writeRequest(descriptor.handle, data, false), data => {
    ...
    if (opcode === ATT_OP_WRITE_RESP) {
      this.emit('valueWrite', this._id, serviceUuid, characteristicUuid, descriptorUuid);
    }
  });
};
  ...
  const callback = (data) => {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('handleRead', this._id, handle, readData);
      }
    } else {
      this.emit('handleRead', this._id, handle, readData);
    }
...
Gatt.prototype.writeHandle = function (handle, data, withoutResponse) {
  if (withoutResponse) {
    this._queueCommand(this.writeRequest(handle, data, true), null, () => {
      this.emit('handleWrite', this._id, handle);
    });
  } else {
