...
class Peripheral extends events.EventEmitter {
  ...
  addService(service){
    ...
    const serv = new Service(this._noble, this.id, service.uuid);
  ...
  _getService(id){
    ...
    if(!service) throw new Service(`unknown service`)
    return service
  }
  ...
  onServicesDiscover(serviceUuids) {
    const ret = []
    for(const sIn of serviceUuids){
      let sObj = this.getService(sIn)
      if(!sObj) sObj = this.addService(sIn)
  ...
  onCharacteristicsDiscovered(serviceUuid, characteristics) {
    const service = this._getService(serviceUuid)
    return service.onCharacteristicsDiscovered(characteristics)
  }
  ...
  onNotify(serviceUuid, characteristicUuid, state) {
    const service = this._getService(serviceUuid)
    return service.onNotify(characteristicUuid, state)
  }
